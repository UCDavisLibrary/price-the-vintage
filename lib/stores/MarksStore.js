...
class MarksStore extends BaseStore {
  ...
  setApprovedByPageLoading(pageId) {
    this.approvedPageRequestState[pageId] = {state: this.STATE.LOAING};
    this.emitApprovedByPageEvent(pageId);
  }

  setApprovedByPageError(pageId, error) {
    this.approvedPageRequestState[pageId] = {state: this.STATE.ERROR, error};
    this.emitApprovedByPageEvent(pageId);
  }

  setApprovedByPageLoaded(marks) {
    this.approvedPageRequestState[pageId] = {state: this.STATE.LOADED, error};
  ...
  emitApprovedByPageEvent(pageId) {
    this.emit(this.PENDING_SEARCH_UPDATE_EVENT, this.approvedPageRequestState[pageId]);
  }

  /**
   * Should already by structured
   * 
   * @param {object} mark - mark data
   * @param {string} mark.state - mark state
   * @param {string} mark.markId - mark id
  ...
  setMark(mark) {
    ...
    this.emitMarkUpdateEvent(data.markId);
  }
  ...
  emitApprovedByPageEvent(pageId) {
    this.emit(this.MARK_UPDATE_EVENT, this.byId[pageId]);
  }
