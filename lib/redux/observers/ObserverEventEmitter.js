...
class ObserverEventEmitter {
  ...
  // user activity
  static onUserActivityChange(state) {
    EventBus.emit('user-activity-update', state);
  }

  // app state
  static onAppStateChange(state) {
    EventBus.emit('app-state-update', state);
  }
  ...
  // catalogs
  static onCatalogSearchChange(state) {
    EventBus.emit('search-catalogs-update', state);
  }

  static onCatalogChange(state) {
    EventBus.emit('catalog-update', state);
  }
