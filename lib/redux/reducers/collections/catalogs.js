...
function setData(state, id, value) {
  if( value.meta.error ) {
    return setError(state, id, value.meta.error);
  }

  state.byId = Object.assign({}, state.byId, {[id]: {id: id, payload: value.data[0], state: 'loaded' }});
...
function catalogs(state = initialState, action) {
  switch (action.type) {
    ...
    case actions.GET_CATALOG_SUCCESS:
      return setData(state, action.id, action.response.body);
