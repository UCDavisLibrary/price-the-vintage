 ...
 * Preform admin tasks
 */
class AdminModel extends ReduxModel {
  ...
  ignorePage(pageId, ignore) {
    return new Promise((resolve, reject) => {
      ...
      if( !user.auth0Token || !user.isAdmin ) {
        return reject({error: true, message: 'Must be admin and have a token'});
      }

      pageService.ignore(pageId, ignore, user.auth0Token, (err, res) => {
        if( err ) return reject(err);
        if( res.length === 0 ) return reject({error: true, message: 'Bad response from server'});
        var page = res[0];
   ...
   * TODO: move this, allowed by users too
   * 
  ...
  pendingMarkSearch(params) {
    ...
    this.dispatch(
      markActions.pendingMarkSearch(params)
    );
